...
void HGE_CALL HGE_Impl::System_SetStateBool(hgeBoolState state, bool value)
{
    switch(state)
    {
        case HGE_WINDOWED:      if(m_vertices || hwndParent) break;
                                if(m_pD3DDevice && bWindowed != value)
                                {
        ...
        case HGE_ZBUFFER:       if(!m_pD3DDevice) bZBuffer=value;
                                break;
								/*
...
void HGE_CALL HGE_Impl::System_SetStateInt(hgeIntState state, int value)
{
    switch(state)
    {
        case HGE_SCREENWIDTH:   if(!m_pD3DDevice) nScreenWidth=value; break;

        case HGE_SCREENHEIGHT:  if(!m_pD3DDevice) nScreenHeight=value; break;

        case HGE_SCREENBPP:     if(!m_pD3DDevice) nScreenBPP=value; break;
...
void HGE_CALL HGE_Impl::System_SetStateString(hgeStringState state, const char *value)
{
    ...
    switch(state)
    {
        case HGE_ICON:          szIcon=value;
                                if(pHGE->hwnd) SetClassLong(pHGE->hwnd, -14, (LONG)LoadIcon(pHGE->hInstance, szIcon));
                                break;
