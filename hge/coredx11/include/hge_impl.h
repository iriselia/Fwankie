...
public:
	...
	//Testing
	virtual void        HGE_CALL    Gfx_Test();
	//
	...
	virtual void		HGE_CALL	Texture_Write(HTEXTURE tex, void* pData);
	...
	ID3D11InputLayout* m_pInputLayout0;
	ID3D11InputLayout* m_pInputLayout1;            // the pointer to the input layout
	ID3D11VertexShader* m_pVS0;               // the pointer to the vertex shader
	ID3D11VertexShader* m_pVS1;               // the pointer to the vertex shader
	ID3D11PixelShader* m_pPS0;                // the pointer to the pixel shader
	ID3D11PixelShader* m_pPS1;                // the pointer to the pixel shader
	//ID3D11Texture2D;

    DXGI_SWAP_CHAIN_DESC*  d3dpp;

	DXGI_SWAP_CHAIN_DESC   d3dppW;
    RECT                    rectW;
    LONG                    styleW;

	DXGI_SWAP_CHAIN_DESC   d3dppFS;
    RECT                    rectFS;
    LONG                    styleFS;
	...
	ID3D11Buffer *m_pVertexBuf;                // the pointer to the vertex buffer
	ID3D11Buffer *m_pIndexBuf;                // the pointer to the index buffer
	...
	ID3D11RenderTargetView *pRTView;    // the pointer to our back buffer
