...
const TCHAR WindowsWindow::AppWindowClass[] = TEXT("UnrealWindow");
...
void WindowsWindow::Initialize(WindowsApplication* const Application, const std::shared_ptr<GenericWindowDefinition> InDefinition, HINSTANCE InHInstance, const std::shared_ptr< WindowsWindow >& InParent, const bool bShowImmediately)
{
	...
	int32 X = PlatformMath::TruncToInt(XInitialRect);
	int32 Y = PlatformMath::TruncToInt(YInitialRect);
	int32 ClientWidth = PlatformMath::TruncToInt(WidthInitial);
	int32 ClientHeight = PlatformMath::TruncToInt(HeightInitial);
...
HRGN WindowsWindow::MakeWindowRegionObject() const
{
	HRGN Region;
...
void WindowsWindow::ReshapeWindow(int32 NewX, int32 NewY, int32 NewWidth, int32 NewHeight)
{
	...
	if (Definition->SizeWillChangeOften)
	{
		...
		NewWidth = PlatformMath::Max(NewWidth, PlatformMath::Min(OldWidth, MinRetainedWidth));
		NewHeight = PlatformMath::Max(NewHeight, PlatformMath::Min(OldHeight, MinRetainedHeight));
	}
...
{
	bool bTrueFullscreen = (WindowMode == EWindowModeType::Fullscreen);
...
void WindowsWindow::SetWindowMode(EWindowModeType NewWindowMode)
{
	if (NewWindowMode != WindowMode)
	{
		bool bTrueFullscreen = NewWindowMode == EWindowModeType::Fullscreen;
		...
		if (NewWindowMode == EWindowModeType::WindowedFullscreen || NewWindowMode == EWindowModeType::Fullscreen)
		{
			...
			LONG TargetClientWidth = bTrueFullscreen ?
				PlatformMath::Min(MonitorWidth, ClientRect.right - ClientRect.left) :
				MonitorWidth;

			LONG MonitorHeight = MonitorInfo.rcMonitor.bottom - MonitorInfo.rcMonitor.top;
			LONG TargetClientHeight = bTrueFullscreen ?
				PlatformMath::Min(MonitorHeight, ClientRect.bottom - ClientRect.top) :
				MonitorHeight;
...
void WindowsWindow::AdjustCachedSize(Vector2D& Size) const
{
	if (Definition.get() && Definition->SizeWillChangeOften)
	{
		Size = Vector2D(VirtualWidth, VirtualHeight);
	}
	else if (HWnd)
	{
		RECT ClientRect;
...
{
	SetLayeredWindowAttributes(HWnd, 0, PlatformMath::TruncToInt(InOpacity * 255.0f), LWA_ALPHA);
}
...
void WindowsWindow::SetText(const TCHAR* const Text)
{
...
DragDropOLEData DecipherOLEData(IDataObject* DataObjectPointer)
{
	...
	{
		OLEResourceGuard ResourceGuard(StorageMedium);
		OLEData.Type = DragDropOLEData::Text;
	...
	{
		OLEData.Type = DragDropOLEData::Files;
		...
		if (DropFiles->fWide)
		{
			...
			while (Pos[0] != 0)
			{
				const std::tstring ListElement = std::tstring(Pos);
				OLEData.OperationFilenames.push_back(ListElement);
				Pos += ListElement.length() + 1;
			}
		}
		else
		{
			...
			while (Pos[0] != 0)
			{
				TCHAR a [100];
				mbstotcs(a, Pos, 100);
				const std::tstring ListElement = std::tstring(a);
				OLEData.OperationFilenames.push_back(ListElement);
				Pos += ListElement.length() + 1;
			}
		}
